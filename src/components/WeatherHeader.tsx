...
export async function WeatherHeader({ data, locations }: WeatherHeaderProps) {
  ...
  return (
    <header className="space-y-4 landscape:space-y-2">
      <div className="flex items-start justify-between gap-3 landscape:gap-2">
        <div className="min-w-0 flex-1">
          <LocationSwitcher locations={locations} selectedId={data.location.id} />
          <div className="mt-1 landscape:flex landscape:flex-wrap landscape:items-baseline landscape:gap-x-3">
            <p className="text-slate-600 dark:text-slate-400">{data.location.region}</p>
            {selectedLocation && (
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400 landscape:mt-0">
                <a
      ...
      <div className="rounded-2xl bg-gradient-to-br from-sky-500 to-sky-700 p-6 text-white shadow-lg landscape:p-4 dark:from-sky-600 dark:to-sky-900 dark:shadow-sky-950/30">
        <div className="flex items-start justify-between gap-4 landscape:gap-3">
          <div>
            <p className="text-sm font-medium text-sky-100">
              {format(current.time, getDatePattern(locale, "headerDateTime"), { locale: dateLocale })}
            </p>
            <p className="mt-2 text-5xl font-bold tabular-nums landscape:mt-1 landscape:text-4xl">
              {Math.round(current.temperature)}°C
            </p>
            <p className="mt-1 text-sky-100 landscape:text-sm">
              <FeelsLikeText
          ...
          <span className="text-5xl landscape:text-4xl" aria-hidden="true">
            {getConditionEmoji(current.iconCode)}
          </span>
        </div>

        <dl className="mt-6 grid grid-cols-2 gap-4 landscape:mt-3 landscape:gap-2 landscape:grid-cols-4 sm:grid-cols-4">
          <div>
